...
const FLAGS: readonly FlagMeta[] = [
  ...
  },
  {
    ...
    description:
      "Enable the background update poll in a dev build so the global update bar picks up a local dev-release.sh feed (D1, local updater endpoint). Each check/install targets the local feed explicitly (#1033), so no FF_UPDATER_ENDPOINT is needed — set it only to override the endpoint (D1 dogfood). Not dev-install.sh (D2, direct file swap, no feed).",
    restartRequired: true,
  },
  {
    id: "devTools",
