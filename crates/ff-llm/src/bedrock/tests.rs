...
/// `chat_stream` composes it — shared guard first, then `to_converse`.
///
/// This is the case the unit tests in lib.rs can't express: the wake's context arrives
...
fn observer_wake_conversation_is_repaired_end_to_end() {
    ...
    let repaired = crate::enforce_user_terminated(raw);
    let (system, messages) = to_converse(&repaired);
    ...
    match &messages.last().unwrap().content[0] {
        ContentBlock::Text(t) => assert_eq!(t, "Continue."),
        other => panic!("expected text block, got {other:?}"),
    }
}

/// #1117 ordering contract, pinned through the real composition point.
/// `prepare_wire_messages` marks cache breakpoints after the shared guard has already
/// repaired the conversation. This asserts the cost consequence rather than the call
/// order: the synthesized nudge is volatile -- a fresh one per background wake -- so
/// it must stay outside the cached prefix. The breakpoint indexes off `messages.len()`,
/// so marking before repairing would fold a per-turn message into the prefix and
/// invalidate the cache on every wake.
#[test]
fn cache_prefix_excludes_the_synthesized_nudge() {
    let text = |t: &str| vec![ContentBlock::Text(t.to_string())];
    // Simulate the upstream repair: 4 raw messages become 5 after enforce_user_terminated.
    let convo = vec![
        ...
        msg(ConversationRole::User, text("Continue.")),
    ];
    ...
    // Original length was 4, so markable = true.
    let out = prepare_wire_messages(convo.clone(), true, true);
    ...
    // Cache disabled: no breakpoints anywhere.
    let plain = prepare_wire_messages(convo, false, true);
    assert!(
...
/// A lone assistant message is repaired to two by the shared guard, which crosses
/// the cache breakpoint's `len() >= 2` threshold. Marking it would put a breakpoint
/// on a 2-message conversation whose prefix is a single volatile turn -- worthless
/// to cache and a behaviour change from before the repair existed. Guard against
/// re-introducing it by measuring `markable` before repair.
#[test]
fn single_assistant_message_is_not_cache_marked_after_repair() {
    // Simulate the upstream repair: 1 raw message becomes 2 after enforce_user_terminated.
    let convo = vec![
        msg(
            ConversationRole::Assistant,
    ...
    // Original length was 1, so markable = false.
    let out = prepare_wire_messages(convo, true, false);

    assert_eq!(out.len(), 2, "repair already applied upstream");
    assert!(
        !out.iter().any(|m| m
            .content()
            .iter()
