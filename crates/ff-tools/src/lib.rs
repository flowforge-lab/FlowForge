//! Built-in tools the agent can call: `bash`, `view`, `edit`, `write`, `grep`, `glob`, `todo`.
//!
...
//!
//! Search/discovery tools ([`grep`], [`glob`]) are read-only and jailed to the
//! same root; [`todo`] is a stateless planning checklist (full-replace).
